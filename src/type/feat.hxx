...
void standardize(std::vector<FVal> &feats,
...
public:
  ...
  virtual void serialize(std::vector<FVal> &feats) const {
    ...
    std::copy(bboxSize.begin(), bboxSize.end(), std::back_inserter(feats));
  }

  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    os << f.area << " " << f.perim << " " << f.compactness << " " << f.bboxArea;
    for (auto x : f.bboxSize) {
      os << " " << x;
    }
    return os;
  }
  ...
  void generate(TPoints const &region, double normalizingArea,
                double normalizingLength) {
    const uint D = TPoints::Point::Dimension;
    init(D);
    area = region.size();
    perim = region.boundary.size() + region.border.size();
    compactness = sdivide(std::pow(perim, (double)D / (D - 1)), area, 0.0);
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    return os << f.areaDiff << " " << f.rAreaDiff0 << " " << f.rAreaDiff1 << " "
              << f.perimDiff << " " << f.rPerimDiff0 << " " << f.rPerimDiff1;
  }

  void generate(RegionShapeFeats const &rf0, RegionShapeFeats const &rf1) {
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    return os << static_cast<Super const &>(f) << f.boundaryLength << " "
              << f.rBoundaryLengthArea0 << " " << f.rBoundaryLengthArea1 << " "
              << f.rBoundaryLengthPerim0 << " " << f.rBoundaryLengthPerim1;
  }

  template <typename TPoints>
  void generate(TPoints const &boundary, RegionShapeFeats const &rf0,
                RegionShapeFeats const &rf1, double normalizingLength) {
    Super::generate(rf0, rf1);
    boundaryLength =
        sdivide(std::ceil(boundary.size() / 2.0), normalizingLength, 0.0);
...
public:
  ...
  virtual void serialize(std::vector<FVal> &feats) const {
    feats.reserve(feats.size() + dim);
    std::copy(centralMoments.begin(), centralMoments.end(),
              std::back_inserter(feats));
    std::copy(huMoments.begin(), huMoments.end(), std::back_inserter(feats));
    feats.push_back(eccentricity);
  }

  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    for (double x : f.centralMoments) {
      os << x << " ";
    }
    for (double x : f.huMoments) {
      os << x << " ";
    }
  ...
  void generate(TRegion<TKey, Point<2>> const &region,
                fPoint<2> const &centroid, double normalizingLength) {
    ...
    eccentricity = alg::getEccentricity(centralMoments[0], centralMoments[2],
                                        centralMoments[4]);
  }
};
...
public:
  ...
  virtual void log() {
    for (double &x : centralMomentDiff) {
      x = slog(x, 0.0);
    }
  }
  ...
  virtual void serialize(std::vector<FVal> &feats) const {
    feats.reserve(feats.size() + dim);
    std::copy(centralMomentDiff.begin(), centralMomentDiff.end(),
              std::back_inserter(feats));
    std::copy(huMomentDiff.begin(), huMomentDiff.end(),
              std::back_inserter(feats));
    feats.push_back(eccentricityDiff);
  }

  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    for (double x : f.centralMomentDiff) {
      os << x << " ";
    }
    for (double x : f.huMomentDiff) {
      os << x << " ";
    }
    return os << f.eccentricityDiff;
  }

  void generate(RegionAdvShapeFeats2D const &rf0,
                RegionAdvShapeFeats2D const &rf1) {
    for (int i = 0; i < 7; ++i) {
      centralMomentDiff[i] =
          std::fabs(rf0.centralMoments[i] - rf1.centralMoments[i]);
      huMomentDiff[i] = std::fabs(rf0.huMoments[i] - rf1.huMoments[i]);
    }
...
public:
  ...
  virtual void serialize(std::vector<FVal> &feats) const {
    feats.reserve(feats.size() + dim);
    std::copy(centroid.begin(), centroid.end(), std::back_inserter(feats));
  }
  ...
  template <typename TPoints>
  void generate(TPoints const &region, double normalizingLength) {
  ...
  void generate(std::vector<double> const &centroid0, int size0,
                std::vector<double> const &centroid1, int size1) {
    for (int i = 0; i < regionDim; ++i) {
      centroid[i] = sdivide(centroid0[i] * size0 + centroid1[i] * size1,
                            size0 + size1, 0.0);
    }
  }
};
...
public:
  ...
  void generate(RegionLocationFeats const &rf0,
                RegionLocationFeats const &rf1) {
    centroidDistL2 = 0.0;
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    return os << f.overlap << " " << f.setDiff0 << " " << f.setDiff1 << " "
              << f.symDiff << " " << f.rOverlapArea0 << " " << f.rOverlapArea1
  ...
  template <typename TPoints, typename TImagePtr>
  void generate(TPoints const &region0, TPoints const &region1,
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    ...
    for (auto x : f.validPerims) {
      os << " " << x;
    }
    for (auto x : f.rValidPerims) {
      os << " " << x;
    }
    return os;
  }
  ...
  void generate(TPoints const &region, double normalizingArea,
    ...
    for (int i = 0; i < N; ++i) {
      ...
      region.boundary.traverse(
          [&vp, &image, &thresholds, i](typename TPoints::Point const &p) {
            if (image->GetPixel(p) >= thresholds[i]) {
              ++vp;
            }
          });
...
public:
  ...
  void log() override {
    Super::log();
    for (auto &x : validBoundaryLengths) {
      x = slog(x, 0.0);
    }
  ...
  void serialize(std::vector<FVal> &feats) const override {
    feats.reserve(feats.size() + dim);
    Super::serialize(feats);
    std::copy(validBoundaryLengths.begin(), validBoundaryLengths.end(),
              std::back_inserter(feats));
    std::copy(rValidBoundaryLengths.begin(), rValidBoundaryLengths.end(),
              std::back_inserter(feats));
    std::copy(rValidBoundaryLengthPerims0.begin(),
              rValidBoundaryLengthPerims0.end(), std::back_inserter(feats));
    std::copy(rValidBoundaryLengthPerims1.begin(),
              rValidBoundaryLengthPerims1.end(), std::back_inserter(feats));
  }

  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    os << static_cast<Super const &>(f);
    for (auto x : f.validBoundaryLengths) {
      os << " " << x;
    }
    for (auto x : f.rValidBoundaryLengths) {
      os << " " << x;
    }
    for (auto x : f.rValidBoundaryLengthPerims0) {
      os << " " << x;
    }
    for (auto x : f.rValidBoundaryLengthPerims1) {
      os << " " << x;
    }
    return os;
  }
  ...
  template <typename TPoints, typename TImagePtr>
  void generate(TPoints const &boundary, double normalizingLength,
                RegionShapeFeats const &rf0, RegionShapeFeats const &rf1,
                TImagePtr const &image, std::vector<double> const &thresholds) {
    ...
    for (int i = 0; i < nThreshold; ++i) {
      ...
      validBoundaryLengths[i] =
          sdivide(std::ceil(vp / 2.0), normalizingLength, 0.0);
      rValidBoundaryLengths[i] =
          sdivide(validBoundaryLengths[i], Super::boundaryLength, 0.0);
      rValidBoundaryLengthPerims0[i] =
          sdivide(validBoundaryLengths[i], rf0.perim, 0.0);
      rValidBoundaryLengthPerims1[i] =
          sdivide(validBoundaryLengths[i], rf1.perim, 0.0);
    }
  }
};
...
public:
  ...
  virtual void serialize(std::vector<FVal> &feats) const {
    feats.reserve(feats.size() + dim);
#ifdef GLIA_USE_HISTOGRAM_AS_FEATS
    std::copy(histogram.begin(), histogram.end(), std::back_inserter(feats));
#endif
  ...
  template <typename TPoints, typename TImagePtr>
  void generate(TPoints const &points, TImagePtr const &image, uint histBin,
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    return os << f.histDistL1 << " " << f.histDistX2 << " " << f.entropyDiff;
  }
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    ...
    os << f.median << " ";
#endif
    return os << f.mean << " " << f.stddev << " " << f.min << " " << f.max;
  }

  template <typename TPoints, typename TImagePtr>
  void generate(TPoints const &points, TImagePtr const &image) {
    int n = points.size();
    if (n == 0) {
      return;
    }
    ...
    vals.reserve(n);
    points.traverse([&image, &vals, this](typename TPoints::Point const &p) {
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os, Self const &f) {
    ...
    os << f.medianDiff << " ";
#endif
    return os << f.meanDiff << " " << f.stdDiff << " " << f.minDiff << " "
              << f.maxDiff;
  }

  virtual void generate(ImageRealFeats const &if0, ImageRealFeats const &if1) {
#ifdef GLIA_USE_MEDIAN_AS_FEATS
...
// Image features
class ImageFeats : public virtual ImageLabelFeats,
...
class ImageDiffFeats : public virtual ImageLabelDiffFeats,
